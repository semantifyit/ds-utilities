...
describe("v5 - getDsDescription()", () => {
  test("correct root node with schema:description", () => {
    ...
    expect(dsu.getDsDescription(dsDs0 as unknown as DsV5)).toBe("Airport Domain Specification is a pattern for annotating airports using schema.org vocabulary. The goal is to define a set of types and properties needed for semantic annotation.");
  });
  test("no root node", () => {
